...
function getProductsForRole(role = currentRole) {
    ...
    if (role === 'cucina') {
        return all.filter(p => {
            ...
            return dept === 'cucina' || (dept === '' && cat === 'cucina') || dept === 'entrambi' || dept === 'tutti';
        });
    } else if (role === 'bar') {
        return all.filter(p => {
            const dept = (p.department || '').toLowerCase();
            const cat = (p.category || '').toLowerCase();
            return dept === 'bar' || (dept === '' && cat === 'beverage') || dept === 'entrambi' || dept === 'tutti';
        });
    } else if (role === 'bowling') {
        return all.filter(p => {
            const dept = (p.department || '').toLowerCase();
            const cat = (p.category || '').toLowerCase();
            return dept === 'bowling' || (dept === '' && cat === 'bowling') || dept === 'entrambi' || dept === 'tutti';
        });
    }
    return all;
}
...
function handleSearch() {
    ...
    searchTimeout = setTimeout(() => {
        ...
        const filteredProducts = roleProducts.filter(product =>
...
function updateProductDepartmentCounters() {
    ...
    const countConsumabili = products.filter(p => {
        const d = (p.department || '').toLowerCase();
        return d === 'entrambi' || (d !== 'cucina' && d !== 'bar' && d !== 'bowling');
    }).length;
...
function confirmResetQuantities() {
    ...
    getProductsForRole(currentRole).forEach(p => {
        p.quantity = 0;
        p.status = 'out-of-stock';
    });
...
function renderSnapshotsList() {
    ...
    container.innerHTML = snapshots.slice().reverse().map(s => {
        const dateObj = new Date(s.date);
        const dateFormatted = isNaN(dateObj) ? s.date : dateObj.toLocaleDateString('it-IT', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
        const opLabel = s.operator || 'Snapshot';
        ...
        return `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 1rem; border: 1px solid var(--border); border-radius: 8px; margin-bottom: 0.5rem; background: #fdfdfd;">
                <div>
                    <strong>${opLabel} - ${dateFormatted}</strong>
                    <div style="font-size: 0.85rem; color: var(--gray);">${s.products ? s.products.length : (s.productsCount || 0)} Prodotti ${s.totalValue ? `· ${currency} ${Number(s.totalValue).toFixed(2)}` : ''}</div>
                </div>
                <div class="table-actions">
...
function deleteSnapshot(id) {
    if (confirm("Eliminare questo snapshot dall'archivio?")) {
